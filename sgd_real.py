...
each=[0.01,0.001,1]
lamb=0.1
writer=open('real_sgd_output.csv','w')
writer.write('M,Lambda,Eta,ValidationERMS,TestingERMS\n')
for eta in each:
	threshold=eta**2
	writer.write(str(clusterNum)+','+str(lamb)+','+str(eta)+',')
	newTrainResult = stochasticTrain(part.training['x_matrix'],part.training['target_vector'], clusterNum=clusterNum,n=0.5,lamb=lamb, eta=eta, threshold)
